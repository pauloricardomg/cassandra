...
| @disable_top_level_bloom_filter@    | LeveledCompactionStrategy       | false       | Supresses bloom filter creation on top level SSTables of this table. In a workload with a large read hit rate (few key misses), the bloom filter of top level SSTables may be supressed to reduce memory footprint without degrading performance significantly, since most of the time the data will be present in the top level SSTables and the bloom filter optimization will not be useful in these cases. Option will be valid for new compacted sstables.  |
