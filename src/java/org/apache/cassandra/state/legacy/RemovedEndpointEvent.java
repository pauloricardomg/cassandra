...
import org.apache.cassandra.state.token.PurgeTokenEvent;
import org.apache.cassandra.state.token.TokenEvent;

public class RemovedEndpointEvent extends LegacyEvent
{
    final InetAddressAndPort node;
    final long expireTime;

    public RemovedEndpointEvent(InetAddressAndPort node, long expireTime)
    {
        this.node = node;
    ...
    {
        return snapshot.getTokens(node).stream().map(t -> new PurgeTokenEvent(t)).collect(Collectors.toList());
    }
}