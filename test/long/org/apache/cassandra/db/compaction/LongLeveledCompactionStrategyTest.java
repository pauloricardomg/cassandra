...
public class LongLeveledCompactionStrategyTest
{
    ...
    @Test
    public void testAntiCompactionWithSkipTopLevelBloomFilterOption() throws Exception
    {
        //enable "skip_top_level_bloom_filter" option
        ColumnFamilyStore cfs = Keyspace.open(KEYSPACE1).getColumnFamilyStore(CF_STANDARDLVL);
        setSkipTopLevelBloomFilterOption(cfs, true);
        ...
        //perform anti-compaction
        Collection<SSTableReader> sstables = AntiCompactionTest.getUnrepairedSSTables(cfs);
        Range<Token> range = new Range<Token>(new ByteOrderedPartitioner.BytesToken("0".getBytes()), new ByteOrderedPartitioner.BytesToken("9999".getBytes()));
        ...
        //Now, all anti-compacted sstables from lower levels should have bloom filters,
        // while top level sstables must still not have bloom filters.
        assertTopLevelSStablesDoNotHaveBloomFilterAndLowerLevelSStablesHave();
