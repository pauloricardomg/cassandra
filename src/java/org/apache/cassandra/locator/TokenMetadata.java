...
public class TokenMetadata
{
    ...
    private final BiMap<InetAddress, InetAddress> replaceEndpoints = HashBiMap.create();
    ...
    public void updateNormalTokens(Multimap<InetAddress, Token> endpointTokens)
    {
        ...
        try
        {
            ...
            for (InetAddress endpoint : endpointTokens.keySet())
            {
                ...
                replaceEndpoints.remove(endpoint);
    ...
    {
        addBootstrapTokens(tokens, endpoint, false);
    }

    private void addBootstrapTokens(Collection<Token> tokens, InetAddress endpoint, boolean replace)
    {
        assert tokens != null && !tokens.isEmpty();
        assert endpoint != null;
        ...
        try
        {
            ...
            for (Token token : tokens)
            {
                ...
                if (!replace && oldEndpoint != null && !oldEndpoint.equals(endpoint))
                    throw new RuntimeException("Bootstrap Token collision between " + oldEndpoint + " and " + endpoint + " (token " + token);
            }
    ...
    public void addReplaceTokens(Collection<Token> replacingTokens, InetAddress newNode, InetAddress oldNode)
    {
        ...
        try
        {
            ...
            replaceEndpoints.put(newNode, oldNode);

            addBootstrapTokens(replacingTokens, newNode, true);
        }
        finally
        {
            lock.writeLock().unlock();
    ...
    {
        return Optional.fromNullable(replaceEndpoints.inverse().get(endpoint));
    }

    public Optional<InetAddress> getReplacingNode(InetAddress endpoint)
    {
        return Optional.fromNullable((replaceEndpoints.get(endpoint)));
    }
    ...
    public void removeEndpoint(InetAddress endpoint)
    {
        ...
        try
        {
            ...
            leavingEndpoints.remove(endpoint);
            if (replaceEndpoints.remove(endpoint) != null)
            {
