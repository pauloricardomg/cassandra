...
| @disable_top_level_bloom_filter@    | LeveledCompactionStrategy       | false       | Supresses bloom filter creation on top level SSTables of this table. In a workload with a high read hit rate, the bloom filter of top level SSTables may be supressed to reduce memory footprint without degrading performance, since most data will be present in top-level sstables without the need to query the bloom filter. Option will be valid for new compacted sstables.  |
