...
public class Repair extends NodeToolCmd
{
    ...
    public void execute(NodeProbe probe)
    {
        ...
        if (keyspaces.size() != 1 && cfnames.length != 1)
            throw new RuntimeException("MBR should specify only one keyspace and table.");

        try
        {
            probe.enableMutationBasedRepair(keyspaces.get(0), cfnames[0], 1000, Integer.MAX_VALUE);
        }
        catch (Exception e)
        {
