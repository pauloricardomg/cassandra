                                 ...
                                 LifecycleTransaction anticompactionGroup, long repairedAt)
    {
             ...
             CompactionIterator ci = new CompactionIterator(OperationType.ANTICOMPACTION, scanners.scanners, controller, nowInSec, UUIDGen.getTimeUUID(), metrics))
        {
            ...
            while (ci.hasNext())
            {
                if (ci.isStopRequested() || Thread.currentThread().isInterrupted())
                {
                    //logger.debug("GOT HERE muthafuckka");
                    throw new CompactionInterruptedException(ci.getCompactionInfo());
                }
                try (UnfilteredRowIterator partition = ci.next())
                {
        ...
        catch (Throwable e)
        {
            ...
            if (!ActiveRepairService.instance.isActive(parentRepairSession))
            {
                throw e; //anticompaction was aborted, rethrow so we stop work
            }
            else
            {
                logger.error("[{}] Error during anticompaction", anticompactionGroup.opId(), e);
            }
        }
