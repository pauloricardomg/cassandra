 ...
 * {@link this#writeLock}.
 *
 * Whenever the {@link DiskBoundaries} or {@link CFMetaData} change, the compaction strategies must be reloaded,
 * so in order to ensure the compaction strategy placement reflect most up-to-date table metadata and disk boundaries,
 * call {@link this#maybeReload()} before acquiring the read lock to acess the strategies.
 *
 */
...
public class CompactionStrategyManager implements INotificationConsumer
{
    ...
    {
        return maybeReload(metadata.params.compaction);
    }

    private boolean maybeReload(CompactionParams newCompactionParams)
    {
        if (!shouldReload(newCompactionParams))
            return false;
    ...
    /**
     * Checks if the disk boundaries or table metadata changed and reloads the compaction strategies
     * to reflect the most up-to-date table metadata and disk boundaries.
     *
     * This is typically called before acquiring the {@link this#readLock} to ensure the most up-to-date
     * disk locations, boundaries and table metadata are used.
     *
    ...
    @VisibleForTesting
    private boolean maybeReload()
    {
        return maybeReload(params);
    }

    private boolean shouldReload(CompactionParams newCompactionParams)
    {
        return !newCompactionParams.equals(schemaCompactionParams) || // compare the old schema configuration to the new one, ignore any locally set changes.
