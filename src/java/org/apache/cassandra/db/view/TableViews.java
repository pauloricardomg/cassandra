...
public class TableViews extends AbstractCollection<View>
{
     ...
     */
    public ViewWriteHandler pushViewReplicaUpdates(PartitionUpdate update)
    {
        assert update.metadata().id.equals(baseTableMetadata.id);

        Collection<View> views = updatedViews(update);
        if (views.isEmpty())
            return ViewWriteHandler.EMPTY;
        ...
        Collection<Mutation> viewMutations;
        ...
        {
            viewMutations = Iterators.getOnlyElement(generateViewUpdates(views, updates, existings, nowInSec, false));
        }
        ...
        Mutation baseMutation = new Mutation(update);
        baseAndViews.add(baseMutation); //base mutation
        baseAndViews.addAll(viewMutations);

        // if we haven't joined the ring, write everything to batchlog because paired replicas may be stale
        if (StorageService.instance.isStarting() || StorageService.instance.isJoining() || StorageService.instance.isMoving())
        {
            AtomicBatchWriter.storeBatchlog(baseAndViews);
            return ViewWriteHandler.EMPTY;
        }

        AtomicLong baseComplete = new AtomicLong(Long.MAX_VALUE);

        AtomicBatchWriter.AtomicBatchWriteHandler atomicBatchWriteHandler = StorageProxy.instance.mutateViewsAtomicallyAsync(queryStartNanoTime,
                                                                                                                             baseMutation, baseAndViews,
                                                                                                                             ViewUtils.viewEndpointSelector(update.partitionKey().getToken()),
                                                                                                                             baseComplete);
        return new ViewWriteHandler(baseComplete, atomicBatchWriteHandler);
    }
     ...
     * passed to {@link #pushViewReplicaUpdates(PartitionUpdate)}.
     *
