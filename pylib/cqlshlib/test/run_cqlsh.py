...
if is_win():
    from pseudotty import PseudoTTY
else:
    import pty

DEFAULT_CQLSH_PROMPT = '(\S+@)?cqlsh(:\S+)?> '
...
class ProcRunner:
    ...
    def start_proc(self):
        ...
        if self.realtty:
            ...
            if self.tty:
                self.pseudotty = PseudoTTY(self.proc.stdout)
                self.read = self.read_pseudotty
            else:
                self.read = self.read_tty
    ...
    def read_pseudotty(self, blksize, timeout=None):
        return self.pseudotty.read(blksize, timeout)
...
class CqlshRunner(ProcRunner):
    def __init__(self, path=None, host=None, port=None, keyspace=None, cqlver=None,
                 ...
                 win_force_colors=True, tty=None, **kwargs):
        if path is None:
            cqlsh_bin = 'cqlsh'
            if is_win():
                cqlsh_bin = 'cqlsh.bat'
            path = normpath(join(basecase.cqlsh_dir, cqlsh_bin))
        if host is None:
        ...
        env.setdefault('PYTHONUNBUFFERED', os.environ.get('PYTHONPATH', 'true'))
    ...
    def cmd_and_response(self, cmd):
        ...
        try:
            ...
            promptline = output
            output = ''
        assert re.match(self.prompt, promptline), \
