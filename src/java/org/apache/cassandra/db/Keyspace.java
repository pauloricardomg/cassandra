...
public class Keyspace
{
    ...
    {
        return applyInternal(mutation, writeCommitLog, updateIndexes, true, true, new CompletableFuture<>(), true);
    }
    ...
    public void apply(final Mutation mutation,
                      ...
                      boolean isDroppable)
    {
        apply(mutation, writeCommitLog, updateIndexes, isDroppable, true);
    }
                                               ...
                                               boolean updateViews)
    {
        ...
        if (updateViews && updateIndexes && viewManager.updatesAffectView(Collections.singleton(mutation), false))
        {
            return applyMutationAndViews(mutation, writeCommitLog, updateIndexes, isDroppable, isDeferrable, future);
        }
    ...
    private CompletableFuture<?> applyMutationAndViews(Mutation mutation, boolean writeCommitLog, boolean updateIndexes,
                                                       boolean isDroppable, boolean isDeferrable, CompletableFuture<?> future)
    {
        Lock[] locks = getViewLocks(mutation, isDroppable, isDeferrable);
        ...
        try
        {
            for (PartitionUpdate upd : mutation.getPartitionUpdates())
            {
                ...
                try
                {
                    Mutation baseMutation = new Mutation(upd);
                    Collection<Mutation> viewMutations = viewManager.forTable(upd.metadata().id).generateViewReplicaUpdates(upd);
                    if (viewMutations.isEmpty())
                    {
                        applyInternal(baseMutation, writeCommitLog, updateIndexes, isDroppable, isDeferrable, future, false);
                    }
                    else
                    {
                        ArrayList<Mutation> baseAndViews = new ArrayList<>(viewMutations.size() + 1);
                        baseAndViews.add(baseMutation);
                        baseAndViews.addAll(viewMutations);
                        if (StorageService.instance.isStarting() || StorageService.instance.isJoining() || StorageService.instance.isMoving())
                        {
                            // if we haven't joined the ring, write everything to batchlog because paired replicas may be stale
                            StorageProxy.writeViewBatchlog(baseAndViews);
                            applyInternal(mutation, writeCommitLog, updateIndexes, isDroppable, isDeferrable, future, false);
                        }
                        else
                        {
                            StorageProxy.mutateBaseAndViewsAtomically(upd.partitionKey().getToken(), baseAndViews);
                        }
                    }
                }
                catch (Throwable t)
    ...
    private Lock[] getViewLocks(Mutation mutation, boolean isDroppable, boolean isDeferrable)
    {
