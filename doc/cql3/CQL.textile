...
| @disable_top_level_bloom_filter@    | LeveledCompactionStrategy       | false       | Does not load bloom filters of top level SSTables in memory. In a workload where all reads are known to be present, this optimization may considerably reduce memory footprint without degrading read performance. |
