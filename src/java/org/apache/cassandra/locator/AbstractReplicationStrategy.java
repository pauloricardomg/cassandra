...
public abstract class AbstractReplicationStrategy
{
                                                                       ...
                                                                       Predicate<InetAddress> isAlive)
    {
        if (consistency_level.isDatacenterLocal())
        {
            ...
            return new DatacenterWriteResponseHandler<T>(naturalEndpoints, pendingEndpoints, consistency_level, getKeyspace(), callback, writeType, isAlive);
        }
        else if (consistency_level == ConsistencyLevel.EACH_QUORUM && (this instanceof NetworkTopologyStrategy))
        {
            return new DatacenterSyncWriteResponseHandler<T>(naturalEndpoints, pendingEndpoints, consistency_level, getKeyspace(), callback, writeType, isAlive);
        }
        else
        {
            return new WriteResponseHandler<T>(naturalEndpoints, pendingEndpoints, consistency_level, getKeyspace(), callback, writeType, isAlive);
        }
    }
