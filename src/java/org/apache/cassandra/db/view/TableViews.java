...
public class TableViews extends AbstractCollection<View>
{
     ...
     */
    public ViewWriteFutures pushViewReplicaUpdates(PartitionUpdate update)
    {
        assert update.metadata().id.equals(baseTableMetadata.id);

        Collection<View> views = updatedViews(update);
        if (views.isEmpty())
            return ViewWriteFutures.EMPTY;
        ...
        AtomicBatchWriter.AtomicBatchWriteHandler atomicBatchWriteHandler = AtomicBatchWriter.mutateViewsAtomically(queryStartNanoTime,
                                                                                                                    baseAndViews, m -> m != baseMutation,
                                                                                                                    ViewUtils.viewEndpointSelector(update.partitionKey().getToken()));
        return new ViewWriteFutures(atomicBatchWriteHandler);
    }
