...
/**
 * This suite checks that data inserted with a TTL overflowing {@link AbstractCell#localDeletionTime()}
 * is readable after inserted, both in the current version and in SSTables affected by CASSANDRA-14092.
 */
public class LocalExpirationTimeOverflowTest extends CQLTester
{
    ...
    /* Check that entry inserted with MAX_TTL on SSTable with negative localDeletionTime
     * written before CASSANDRA-14092 on table with simple and no clustering columns
     * is present on read */
    ...
    /* Check that entry inserted with MAX_TTL on SSTable with negative localDeletionTime
     * written before CASSANDRA-14092 on table with simple and clustering columns
     * is present on read */
    ...
    /* Check that entry inserted with MAX_TTL on SSTable with negative localDeletionTime
     * written before CASSANDRA-14092 on table with complex and no clustering columns
     * is present on read */
    ...
    /* Check that entry inserted with MAX_TTL on SSTable with negative localDeletionTime
     * written before CASSANDRA-14092 on table with complex and clustering columns
     * is present on read */
